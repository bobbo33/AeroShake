...
; Author Contact: bobbo33 on http://groups.google.com/group/lifehacker-coders
;
; Script Function:
...
;v1.5 2009-06-28  Add shake_maxdistance and shake_resetinterval (thanks haha01haha01!)
;                 Add GNU GPL license info
...
SetTimer,RESETCOUNT,%shake_resetinterval%
MouseGetPos, x
xstart := x
xstate := 0
xstate_prev := xstate
...
;runs when shake_resetinterval is exceeded
RESETCOUNT:
xcount := 0
return 
...
CHECKMOUSESHAKE:
   ...
   { 
      ...
      if ( windowstate = 0)
      {
         ...
         xstate_prev := xstate
         MouseGetPos, x, y
         if ( (x-xstart) > shake_distance  and (x-xstart) < shake_maxdistance )
         {
            ; moving up
            xstate := 1
            xstart := x
         }
         else if ( (xstart-x) > shake_distance and (xstart-x) < shake_maxdistance )
         {
            ; moving down
            xstate := -1
            xstart := x
...
Gui,Destroy
Gui,Font
Gui,Add,Text,xm y,Last update by bobbo33 on 2009-06-28
Gui,Font
...
Gui,Font
Gui,Add,Text,xm y+20,AeroShake is FREE SOFTWARE released under the 
Gui,Font,CBlue Underline
Gui,Add,Text,x+5 GLICENSE,GNU General Public License
Gui,Font
Gui,Add,Button,GABOUTOK Default xm+220 y+20 w75,&OK
Gui,Show,,AeroShake v1.5
about=
Return
...
LICENSE:
Run,http://www.gnu.org/licenses/,,UseErrorLevel
Return
...
IfNotExist,AeroShake.ini
{
  ...
  FileAppend,;Max Distance (in pixels) is how far the mouse has to travel to no longer count as a "shake"`n,AeroShake.ini
  FileAppend,;Max Time (in milliseconds) is the maximum time for detecting a "shake" (any longer is just a normal window drag)`n,AeroShake.ini
  ...
  IniWrite,250,AeroShake.ini,Shake Detection,Max Distance 
  IniWrite,2500,AeroShake.ini,Shake Detection,Max Time 
...
IniRead,temp,AeroShake.ini,Shake Detection,Max Time
